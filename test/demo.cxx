...
template<
    ...
    int aux
    >
void fcompute(queue& Q, const int haloSize, const int sourcePatchSize, const int destPatchSize, double * Qin, double * Qout)
{
  const double dt =0.5;
  const size_t NPT=$XXX;

  //int globalrange = numVPAIP * numVPAIP;
...
int main(int argc, char* argv[])
{
    ...
    //for (int i=0;i<std::atoi(argv[1]);i++) 
      //hcompute<numVPAIP,5,0>(Q, 1, srcPS, destPS ,Xin, Xout);
    //auto end = std::chrono::steady_clock::now();
    //std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()   << " ms" << std::endl;
    //start = std::chrono::steady_clock::now();   
    for (int i=0;i<std::atoi(argv[1]);i++) 
      fcompute<numVPAIP,5,0>(Q, 1, srcPS, destPS ,Xin, Xout);
    auto end = std::chrono::steady_clock::now();
    std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()   << " ms" << std::endl;
    //for (int i=0;i<std::atoi(argv[1]);i++) 
      //qcompute<numVPAIP,5,0>(Q, 1, srcPS, destPS ,Xin, Xout);
    //end = std::chrono::steady_clock::now();
    //std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()   << " ms" << std::endl;
    //for (int i=0;i<std::atoi(argv[1]);i++) 
      //fqcompute<numVPAIP,5,0>(Q, 1, srcPS, destPS ,Xin, Xout);
    //end = std::chrono::steady_clock::now();
    //std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()   << " ms" << std::endl;
